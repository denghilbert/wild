...
class Rotation(object):
    ...
    def rot_z(self, thetaZ):
        data_Z = [1, np.cos(thetaZ), np.sin(thetaZ), 1, -np.sin(thetaZ), np.cos(thetaZ), np.cos(2 * thetaZ),
                  np.sin(2 * thetaZ), np.cos(thetaZ), np.sin(thetaZ), 1, -np.sin(thetaZ), np.cos(thetaZ),
                  -np.sin(2 * thetaZ), np.cos(2 * thetaZ)]

        return scipy.sparse.coo_matrix((data_Z, (self.rows_z, self.cols_z)), shape=(9, 9)).toarray()

    def rot_y(self, thetaY):
        data_Z = [1, np.cos(thetaY), np.sin(thetaY), 1, -np.sin(thetaY), np.cos(thetaY), np.cos(2 * thetaY),
                  np.sin(2 * thetaY), np.cos(thetaY), np.sin(thetaY), 1, -np.sin(thetaY), np.cos(thetaY),
                  -np.sin(2 * thetaY), np.cos(2 * thetaY)]
    ...
    def rot_x(self, thetaX):
        data_Z = [1, np.cos(thetaX), np.sin(thetaX), 1, -np.sin(thetaX), np.cos(thetaX), np.cos(2 * thetaX),
                  np.sin(2 * thetaX), np.cos(thetaX), np.sin(thetaX), 1, -np.sin(thetaX), np.cos(thetaX),
                  -np.sin(2 * thetaX), np.cos(2 * thetaX)]
        rotM_z = scipy.sparse.coo_matrix((data_Z, (self.rows_z, self.cols_z)), shape=(9, 9)).toarray()
...
def sh_recon(nm, lighting):
    ...
    sh_basis = np.stack(
        [c1 * domega, c2 * y * domega, c2 * z * domega, c2 * x * domega, c3 * x * y * domega, c3 * y * z * domega,
         c4 * (3 * z * z - 1) * domega, c3 * x * z * domega, c5 * (x * x - y * y) * domega], axis=1)
...
def sh_recon_torch(nm, lighting):
    width = nm.shape[1]
    ...
    # domega = 4*np.pi**2/width**2 * sinc(zen)
    domega = torch.ones_like(zen)

    sh_basis = torch.stack(
        [c1 * domega, c2 * y * domega, c2 * z * domega, c2 * x * domega, c3 * x * y * domega, c3 * y * z * domega,
         c4 * (3 * z * z - 1) * domega, c3 * x * z * domega, c5 * (x * x - y * y) * domega], axis=1)
    sh_basis = sh_basis.unsqueeze(-1)
    lighting_recon = lighting.unsqueeze(-2).unsqueeze(-2) * sh_basis
    lighting_recon = lighting_recon.sum(dim=1)
...
def main():
    ...
    latlongMap_dirs, latlongMap_values = lmd(exr_img)
    angularMap_dirs, _, angularMap_mask = amd(exr_img_angMap)
    ...
    rot_lighting_recon = (rot_lighting_recon - rot_lighting_validPix.min()) / (
                rot_lighting_validPix.max() - rot_lighting_validPix.min())
    rot_lighting_recon[np.isnan(rot_lighting_recon)] = 0

    # plt.figure();
    # plt.imshow(exr_img)
    # plt.figure();
    # plt.imshow(exr_img_angMap)
    # plt.figure();
    # plt.imshow(lighting_recon[0])
    # plt.figure();
    # plt.imshow(rot_lighting_recon[0])
    # plt.show()

    gt_env = torch.tensor(exr_img_angMap)
    gt_env[gt_env==0] = 1
    lgtSGs = nn.Parameter(torch.randn(9, 3))
    ...
    for step in range(N_iter):
        ...
        lighting_recon[torch.isnan(lighting_recon)] = 0
        # lighting_validPix = lighting_recon[~torch.isnan(lighting_recon)]
        # min, max = lighting_validPix.min().detach(), lighting_validPix.max().detach()
        lighting_recon = (lighting_recon - lighting_recon.detach().min()) / (lighting_recon.detach().max() - lighting_recon.detach().min())

        env_map = lighting_recon[0]
        env_map[env_map==env_map[0][0][0]] = 1
        # plt.figure()
        # plt.imshow(lighting_recon[0].detach().numpy())
        # plt.figure()
        # plt.imshow(gt_env.detach().numpy())
        # plt.show()

        loss = torch.mean((env_map - gt_env) * (env_map - gt_env))
        loss.backward()
        optimizer.step()
