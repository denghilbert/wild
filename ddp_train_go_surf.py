...
from ddp_model_go_surf import NerfNetWithAutoExpo
import time
...
def create_nerf(rank, args):
    ...
    for m in range(models['cascade_level']):
        ...
        optim = torch.optim.Adam([{"params": net.module.nerf_net.decoder.parameters(), "lr": 0.001},
                                      {"params": net.module.nerf_net.grid.parameters(), "lr": 0.01},
                                      {"params": net.module.nerf_net.inv_s, "lr": 0.001}],
...
def ddp_train_nerf(rank, args, one_card=False):
    ...
    for global_step in range(start + 1, start + 1 + args.N_iters):
        ...
        for m in range(models['cascade_level']):
            ...
            if 'normal' in ret:
                eikonal_loss = get_eikonal_loss(ret['normal'])
            else:
                eikonal_loss = torch.tensor(0.0).cuda().float()

            smooth_loss = get_smooth_loss(ret['normal'][:ret['normal'].shape[0] // 2].reshape(-1, 3),
                                          ret['normal'][ret['normal'].shape[0] // 2:].reshape(-1, 3))
