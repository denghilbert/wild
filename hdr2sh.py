...
def main():
    ...
    exr_img_pathss = glob.glob(os.path.join('/home/youmingdeng/youming8/data/lwp/final/ENV_MAP_CC/*/*'))
    names = glob.glob(os.path.join('/home/youmingdeng/youming8/data/lwp/final/ENV_MAP_CC/*'))
    for exr_img_paths, name in zip(exr_img_pathss, names):
        # read and resize the hdr image
        # exr_img = imageio.imread(exr_img_paths, format='EXR-FI')
        exr_img = imageio.imread(exr_img_paths).astype('float32') / 255
        ...
        dest = exr_img_paths.split('CC/')[0] + 'CC/' + name.split('CC/')[1] + '.txt'
        np.savetxt(dest, img_sh)
